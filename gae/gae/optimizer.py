...
class OptimizerVAE(object):
    def __init__(self, preds, labels, model, num_nodes, pos_weight, norm):
        ...
        if FLAGS.vae:
            self.kl = (0.5 / num_nodes) * tf.reduce_mean(tf.reduce_sum(1 + 2 * model.z_log_std * model.sample - tf.square(model.z_mean * model.sample) - tf.square(tf.exp(model.z_log_std * model.sample)), 1))
            self.cost -= self.kl
